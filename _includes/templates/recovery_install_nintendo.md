...
2. Please download the following files from [here](https://updater.oddsolutions.us/devices/{{ device.codename }}) and place them in the noted folders (which you will potentially have to create) on your device's SD Card:
   ...
   * `boot.scr` -> `switchroot/android/boot.scr`
    {% include alerts/warning.html content="Please ensure that the name, path, and case of these files all match the above list identically. Also note that `switchroot` is the name of a **folder** on the SD Card, and **not** the SD Card itself." %}
